...
| `update_event` | `event_id: str`, `subject?: str`, `start?: str`, `end?: str`, `location?: str`, `body?: str`, `attendees?: list[str]`, `optional_attendees?: list[str]`, `send_invitations?: bool = true` | Меняет встречу; только организатор. Передаются лишь изменяемые поля; текстовые поля очистить нельзя. Списки участников **заменяются целиком** (убрать одного из трёх — передать двоих оставшихся), `[]` очищает список, отсутствие параметра оставляет как есть. Только `start` — перенос с сохранением длительности; только `end` — изменение длительности. Уведомления уходят только затронутым изменением (`SendToChangedAndSaveCopy`), как в Outlook. Для серии передаётся `event_id` конкретного экземпляра; id самой серии отклоняется. `event_id` в ответе может отличаться от переданного |
...
> **Если день «обрезается» в середине.** Ответ `list_events` за загруженный
> день — около 9 КБ, и клиент может резать результаты инструментов по своему
> лимиту (в Cline `next` это 8000 символов, переменная
> `CLINE_MESSAGE_BUILDER_MAX_TOOL_RESULT_CHARS`; у других клиентов лимит свой
> или его нет). Это ограничение клиента, не сервера: поднимите лимит в
> настройках своего клиента или запрашивайте меньший диапазон дат.
