...
- **`update_event` уведомляет только затронутых
  (`SendToChangedAndSaveCopy`), а не всех.** Изначально стоял
  `SendToAllAndSaveCopy` — заменено по замечанию пользователя: из интерфейса
  Outlook правка встречи уведомляет только тех, кого изменение касается
  (удалённому — отмена, оставшимся — обновление), и tool не должен быть
  шумнее интерфейса, который он заменяет, иначе каждый перенос рассылает
  всем участникам лишнее письмо. Значение допустимо **только при
  обновлении**: `UpdateItem` валидирует по
  `SEND_MEETING_INVITATIONS_AND_CANCELLATIONS_CHOICES` (есть
  `SendOnlyToChanged`/`SendToChangedAndSaveCopy`), а `create`/`delete` — по
  узким наборам без них, поэтому там остаётся `SendToAllAndSaveCopy`.
