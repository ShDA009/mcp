...
- **`translate_ews_error` не должен сваливать всё в «check VPN/network».**
  ...
  и это дважды направляло диагностику по ложному следу: (1) HTTP 403 от
  прокси (запрос вообще не доходил до Exchange) выглядел как отсутствие VPN,
  хотя VPN работал; (2) `ErrorExceededFindCountLimit` (слишком большой
  результат) — тоже как проблема сети. Теперь: HTTP 401/403 →
  `AuthenticationError` с кодом в тексте, `ErrorExceededFindCountLimit` →
  `ResultTooLargeError` («narrow the date range»), прочие `TransportError`
  сохраняют исходный текст исключения в сообщении. `_http_status` достаёт
  код из текста `MalformedResponseError` (exchangelib кладёт его туда как
  `Code: 403`). `build_account` больше не дублирует перевод, а зовёт
  `translate_ews_error`.
...
- **`list_events` отдаёт `attendees_count`, а не список участников.** Найдено
  на реальном календаре: день из 9 встреч (у части — по 10–16 участников)
  обрезался клиентом на 4-м событии, середина заменялась на
  `...[truncated]...`, и события 3–8 были недоступны LLM вообще.
  Замерено на модели этого дня: полный ответ — **32 198 символов** в худшем
  (и типичном для MCP-клиентов) варианте сериализации — pretty-print с
  `ensure_ascii=True`, где кириллица раздувается в `\uXXXX`. Тот же ответ
  компактно и без экранирования — 15 458, то есть **сериализация удваивает
  размер**, и на неё повлиять нельзя. Участники давали ~70% объёма: 96
  человек × ~150 символов (ФИО + email + статус). После замены на счётчик —
  **8 346 символов**, запас до лимита ~5.8x.
  Рассматривались и отвергнуты: (1) `offset`-пагинация — делит по
  количеству, а проблема в размере, одно событие с 50 участниками всё равно
  не влезет; (2) первые N участников + счётчик — ответ становится неполным
  незаметно для читателя; (3) флаг `include_attendees` — переносит решение
  на LLM, а при `true` проблема возвращается целиком.
  Цена решения: «кто на этой встрече?» стоит дополнительного `get_event`.
  Это осознанный размен — сводка дня обязана помещаться в ответ всегда.
...
- **`update_event` идёт через прямой `UpdateItem`, а не `item.save()` —
  ради копии в «Отправленных».** `Item.save()` жёстко зашивает
  `MessageDisposition="SaveOnly"` (параметра нет, см. исходники
  exchangelib): уведомления уходят, но копия письма никуда не кладётся — в
  отличие от той же правки, сделанной руками в Outlook. Поэтому
  `_save_update` вызывает `UpdateItem` напрямую.
  **Копия появляется только при связке `SendAndSaveCopy` +
  `SendToAllAndSaveCopy`.** Установлено опытом на живом ящике — одна встреча,
  три правки, менялся один параметр за раз:
  `SaveOnly`+`SendToChangedAndSaveCopy` → копии нет;
  `SendAndSaveCopy`+`SendToChangedAndSaveCopy` → копии нет;
  `SendAndSaveCopy`+`SendToAllAndSaveCopy` → копия есть.
  Отсюда цена решения: **уведомление получают все участники, а не только
  затронутые правкой**. Сам Outlook уведомляет точечно (проверено по
  «Отправленным»: при замене участника ушли два письма —
  `IPM.Schedule.Meeting.Request` новому и `IPM.Schedule.Meeting.Canceled`
  выбывшему), но он **сам формирует и шлёт письма**, а не поручает рассылку
  серверу через `UpdateItem` — поэтому у него получается и точечность, и
  копии. Повторить это через один EWS-вызов нельзя; выбран приоритет копии
  (решение пользователя). Промежуточный вариант `SendToChangedAndSaveCopy`
  был реализован и откачен — он давал точечность, но терял копию.
- **`UpdateItem` не записывает новый id обратно в item.** `save()` это
  делает, прямой вызов сервиса — нет. `_save_update` присваивает
  `item._id` из ответа сам, иначе `event_id` в ответе tool указывает на
  версию, которой уже нет (для экземпляров серий id меняется всегда).
- **Списки участников заменяются целиком, `[]` очищает.** `attendees` и
  `optional_attendees` — не «добавить»/«убрать»: чтобы убрать одного из
  трёх, передаётся двое оставшихся (в docstring сказано прямо, иначе LLM
  передаёт удаляемого). `None` — не менять, `[]` — очистить. Раньше
  очистка обязательных участников работала **случайно**: `list(attendees)
  or None` превращал `[]` в `None`, что exchangelib трактует как пустое
  поле; при этом `optional_attendees` в `update_event` не было вовсе, а
  docstring утверждал, что очистка не поддерживается. Теперь это заявленный
  контракт с тестами.
- **`will_send` в `update_event` считается по участникам ДО правки.**
  Иначе очистка списка (`attendees=[]`) давала бы `SendToNone` — удалённые
  участники не получили бы отмену, хотя именно им она и нужна. Проверяется
  `had_attendees or _has_attendees(item)`.
- **`update_event` обязан передавать `update_fields` в `save()`.** Без него
  exchangelib отправляет все поля, которые считает изменёнными, включая
  `uid`, — и Exchange отвергает **весь** запрос на экземпляре серии:
  `ErrorInvalidPropertySet: Single calendar item or recurring master is
  expected (field: calendar:UID)`. Проявлялось так: перенос обычной встречи
  работал, а любая правка `Occurrence`/`Exception` падала. Найдено на живой
  встрече (перенесённый экземпляр повторяющейся серии); диагноз «инструмент
  не поддерживает такой тип события» был **неверен** — `_ensure_writable`
  такие item пропускает, отказ приходил от самого Exchange.
  Теперь `update_event` собирает список реально изменённых полей и передаёт
  его в `save(update_fields=...)`. Побочное следствие: вызов без единого
  поля больше не имеет смысла и отклоняется `InvalidArgumentError`.
  Проверено вживую: перенос экземпляра серии 12:30→14:00 и обратно, с
  сохранением длительности.
