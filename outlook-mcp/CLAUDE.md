...
- **`_find_by_id_in_calendar` тоже обязан быть на `view()`, а не `filter()`.**
  ...
  Известный компромисс: `view()` в окне ±180 дней разворачивает серии в
  отдельные экземпляры, поэтому перебор тяжелее, чем был при `filter()`, и
  идёт без `max_items`. Оставлено сознательно — fallback срабатывает редко
  (только при протухшем/отсутствующем ChangeKey), а сужение окна вернуло бы
  недоступность давних встреч. Если понадобится оптимизация — сужать окно,
  но не возвращаться на `filter()`.
