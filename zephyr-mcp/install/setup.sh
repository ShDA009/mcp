#!/usr/bin/env bash
#
# Установочный скрипт zephyr-mcp (Zephyr Scale / ATM MCP-сервер)
# для macOS (arm64) и Linux.
...
# --- Константы --------------------------------------------------------------
GIT_URL="git+https://github.com/ShDA009/mcp.git#subdirectory=zephyr-mcp"
MCP_ENTRY="zephyr-mcp"
SERVER_KEY="zephyr-scale"
...
info "== Установка zephyr-mcp для $PLATFORM =="
...
CONF_DIR="$HOME/.config/zephyr-mcp"
...
CUR_URL="$(env_get ZEPHYR_BASE_URL || true)"
HAVE_TOKEN="no"; env_get ZEPHYR_API_TOKEN >/dev/null 2>&1 && HAVE_TOKEN="yes"

CHANGE="yes"
if [ -f "$ENV_FILE" ] && [ -n "$CUR_URL" ]; then
  info "Найден существующий конфиг: $ENV_FILE"
  printf '  ZEPHYR_BASE_URL  = %s\n' "${CUR_URL:-<не задан>}"
  printf '  ZEPHYR_API_TOKEN = %s\n' "$([ "$HAVE_TOKEN" = yes ] && echo '******** (сохранён)' || echo '<не задан>')"
...
if [ "$CHANGE" = "yes" ]; then
  info "Введите параметры подключения к Zephyr Scale / ATM (Jira):"
  ZEPHYR_BASE_URL="$(read_nonempty "  Base URL (https://jira.example.com)" "${CUR_URL:-}")"
  # токен — скрытый ввод
  while :; do
    printf '  Personal Access Token (ввод скрыт): '
    if ! read -rs ZEPHYR_API_TOKEN; then
      printf '\n'; die "Ввод прерван (нет данных). Настройка не завершена."
    fi
    printf '\n'
    [ -n "$ZEPHYR_API_TOKEN" ] && break
    warn "Токен не может быть пустым."
  done
else
  # оставляем как есть — перечитаем всё из .env
  ZEPHYR_BASE_URL="$CUR_URL"
  ZEPHYR_API_TOKEN="$(env_get ZEPHYR_API_TOKEN || true)"
...
if "$UVX_BIN" --from "$GIT_URL" "$MCP_ENTRY" --help >/dev/null 2>&1; then
  ...
  warn "  - проверьте, что VPN подключён (для доступа к Jira/ATM)."
fi
...
if [ "$PLATFORM" = "macOS" ]; then
  printf '%s\n' "  3. Если Jira/ATM недоступен — убедитесь, что подключён корпоративный VPN."
else
  printf '%s\n' "  3. Если Jira/ATM недоступен — убедитесь, что подключён корпоративный VPN / есть доступ в корп. сеть."
fi
echo
printf '%s\n' "При проблемах обращайтесь к администратору zephyr-mcp."
